...
const contractDir = '../src/eosio-contracts/eosio.evm'
...
describe('Full Test', () => {
  ...
  it('clears all (dev only, remove in prod)', async () => {
    ...
    const rows = await api.tlos.getAllAddresses(contract)
  ...
  it(`creates new address ${sender.substr(
    ...
    await api.tlos.create({ contract, account, data: arbitrary })
    const rows = await api.tlos.getAllAddresses(contract)
    expect(rows).toEqual([initialAccount])
  })

  it(`transfer 0.0001 TLOS to contract (${contract}) to deposit for ${sender.substr(
    0,
    8
  )} (${account})`, async () => {
    const quantity = '0.0001 TLOS'
    await api.transfer({ from: account, to: contract, quantity, memo: '' })
    const rows = await api.tlos.getAllAddresses(contract)
    expect(rows).toEqual([{ ...initialAccount, balance: quantity }])
  })

  it(`transfer from ${sender.substr(0, 8)} to ${receiver.substr(
    0,
    8
  )}`, async () => {
    const tx = await api.tlos.createEthTx({
      contract,
      sender,
      to: receiver,
      value: '0x01',
